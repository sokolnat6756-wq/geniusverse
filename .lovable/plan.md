## Что меняем
В hero-секции `src/routes/index.tsx` заменяем два CTA («Выбрать Гения» и «Посмотреть тарифы») на одну мягкую кнопку **«Узнать больше»**, которая плавно скроллит к следующей секции «Для кого Академия». Кнопка стилизована с синим фоном `bg-blue-500`.
